...
public class FileHandler implements HttpHandler {
    ...
    private void handleUpload(HttpExchange exchange) throws IOException {
        ...
        SharedFolder folder = config.findFolder(alias);
        if (folder == null) {
            sendError(exchange, 404, "Folder not found");
            return;
        }
        if (folder.isReadOnly()) {
            sendError(exchange, 403, "This folder is read-only.");
            return;
        }
        ...
        try (InputStream is = exchange.getRequestBody()) {
            if (contentType.contains("multipart/form-data")) {
                ...
                if (boundary == null) {
                    sendError(exchange, 400, "Missing boundary");
                    return;
                }
                ...
                if (fileName == null || fileName.isBlank()) {
                    sendError(exchange, 400, "Missing X-File-Name");
                    return;
                }
                ...
                if ("true".equals(isFinish) && uploadId != null) {
                    ...
                    try (RandomAccessFile raf = new RandomAccessFile(partFile.toFile(), "rw");
                        ...
                        if (cl != null)
                            expectedSize = Long.parseLong(cl);

                        long transferred = 0;
                        while (expectedSize < 0 || transferred < expectedSize) {
                            long count = channel.transferFrom(rbc, offset + transferred,
                                    expectedSize < 0 ? Long.MAX_VALUE : (expectedSize - transferred));
                            if (count <= 0)
                                break;
                            transferred += count;
                        }
        ...
        logger.accept("⬆ " + username + " @ " + clientIp + " uploaded " + uploadCount + " file(s) to /" + alias + "/"
                + subPath);
    ...
    private void handleFolderDownload(HttpExchange exchange) throws IOException {
        ...
        if (segments.length == 0 || segments[0].isBlank()) {
            sendError(exchange, 400, "Missing folder");
            return;
        }
        String alias = segments[0];
        ...
        if (!folder.isGuestAccessible() && username == null) {
            sendError(exchange, 403, "Login required");
            return;
        }
        ...
        if (Files.isDirectory(targetPath)) {
            ...
            logger.accept("📦 " + (username != null ? username : "Guest") + " @ " + clientIp
                    + " downloaded folder as ZIP: " + zipName);
    ...
    private int parseMultipartAndSave(InputStream is, String boundary, Path targetDir,
        ...
        while (pos < allBytes.length) {
            ...
            if (pos + 2 <= allBytes.length && allBytes[pos] == '-' && allBytes[pos + 1] == '-')
                break;
            if (pos < allBytes.length && allBytes[pos] == '\r')
                pos++;
    ...
    private String buildDirectoryPage(SharedFolder folder, Path dir, Path root, String subPath,
        ...
        if (!subPath.isBlank()) {
            String parentPath = subPath.contains("/")
                    ...
                    : "";
            parentLink = "<a href=\"" + baseUrl + encode(parentPath)
                    + "\" class=\"file-row parent\">⬆ Parent Directory</a>";
        } else {
    ...
    private String getFileIcon(String name) {
        ...
        if (lower.endsWith(".pdf"))
            return "📄";
        if (lower.matches(".*\\.(jpg|jpeg|png|gif|webp|svg|bmp)"))
            return "🖼️";
        if (lower.matches(".*\\.(mp4|mkv|avi|mov|webm)"))
            return "🎬";
        if (lower.matches(".*\\.(mp3|wav|flac|aac|ogg)"))
            return "🎵";
        if (lower.matches(".*\\.(zip|rar|7z|tar|gz)"))
            return "📦";
        if (lower.matches(".*\\.(doc|docx|odt)"))
            return "📝";
        if (lower.matches(".*\\.(xls|xlsx|csv|ods)"))
            return "📊";
        if (lower.matches(".*\\.(ppt|pptx|odp)"))
            return "📊";
        if (lower.matches(".*\\.(txt|md|log)"))
            return "📃";
        if (lower.matches(".*\\.(java|py|js|ts|html|css|json|xml|sh)"))
            return "💻";
        return "📄";
    }
            ...
            <body>
              ...
              (function(){
                ...
                function uploadFiles(files){
                  ...
                  var MAX_CONCURRENT = 4; // Concurrent chunks for faster upload to SSD/HDD
                  ...
                  function uploadNext() {
                     ...
                     function updateChunkProgress() {
                         ...
                         if (timeDelta > 0.5 || (speedText === '' && timeDelta > 0.1)) {
                             ...
                             if (speedBps > 0) {
                                 var remainingBytes = totalSize - totalLoaded;
                                 var secondsLeft = Math.round(remainingBytes / speedBps);
                                 if (secondsLeft > 60) {
                                     var mins = Math.floor(secondsLeft / 60);
                                     var secs = secondsLeft % 60;
                                     etaText = mins + 'm ' + secs + 's left';
                                 } else {
                                     etaText = secondsLeft + 's left';
                                 }
                             } else {
                                 etaText = 'calculating...';
                             }
                         ...
                         var eta = etaText ? ' - ' + etaText : '';
                         var multiInfo = files.length > 1 ? ' (' + (currentFileIndex+1) + '/' + files.length + ') ' : ' ';
                         progressText.textContent = 'Uploading' + multiInfo + pct + '%' + spd + eta;
                     }
